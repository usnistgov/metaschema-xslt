...
Use mode=concise for one-line report.
Use mode=silent-when-valid for only invalid reports
...
mvn \
    ...
    -Dexec.args="-im:markdown -xsl:${SCRIPT_DIR}/current/computer_inspector.xsl -s:\"$XML_SOURCE\" $ADDITIONAL_ARGS"
