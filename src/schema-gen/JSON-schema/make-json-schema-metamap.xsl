...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="assigned-type" select="(
        $datatype-map[(@as-type = current()/@as-type) and (@prefer = current()/@as-type)],
        $datatype-map[(@as-type = current()/@as-type) and not(@prefer)]
    )[1]/string(.)"/>
