...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template match="define-field[exists(flag | define-flag)]" mode="model">
      ...
      <xsl:for-each-group select="flag | define-flag | $value-property-proxy" group-by="true()"
         expand-text="true">
         <details open="open">
            <summary>{ if (count(current-group()) ne 1) then 'Properties' else 'Property' } ({
               count(current-group()) })</summary>
   ...
   <xsl:template match="*[@as-type = ('markup-line', 'markup-multiline')]"
      mode="get-field-value-name" as="xs:string">RICHTEXT</xsl:template>
   ...
   <xsl:template name="remarks-group">
      ...
      <xsl:for-each-group select="$these-remarks[not(contains-token(@class, 'xml'))]"
         group-by="true()">
   ...
   <xsl:template name="crosslink">
      <div class="crosslink">
         <a class="usa-button" href="{$xml-definitions-link}#{@_metaschema-xml-id}">Switch to
            XML</a>
      </div>
   </xsl:template>

   <xsl:template mode="metaschema-type" match="define-assembly">
      <xsl:text expand-text="true">assembly</xsl:text>
      <br class="br"/>
      <xsl:text> </xsl:text>
   </xsl:template>
