...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!--<xsl:param
   name="schema-path" select="document-uri(/)"/>-->

   <!--  XXX TO DO XXX -->
   <xsl:param
      name="content-converter-path" select="'#'" />

   <xsl:variable name="metaschema-code"
      select="/*/short-name || '-json'" />

   <xsl:variable name="datatype-page" as="xs:string">
   /reference/datatypes</xsl:variable>

   <xsl:strip-space elements="*" />

   <xsl:preserve-space
      elements="p li pre i b em strong a code q" />
   ...
   <xsl:import
      href="../../metapath/docs-metapath.xsl" />

   <xsl:import
      href="../common/metaschema-htmldoc-xslt1.xsl" />

   <xsl:variable name="home" select="/METASCHEMA" />

   <xsl:param
      name="xml-definitions-page" as="xs:string">../../xml/definitions</xsl:param>
   
   
   <xsl:variable
      name="all-references" select="//flag/@name | //model//*/@ref" />

   <xsl:key
      name="assembly-definitions" match="/METASCHEMA/define-assembly" use="@name" />
   <xsl:key
      name="field-definitions" match="/METASCHEMA/define-field" use="@name" />
   <xsl:key
      name="flag-definitions" match="/METASCHEMA/define-flag" use="@name" />
   
   <xsl:key
      name="assembly-references" match="assembly" use="@ref" />
   <xsl:key name="field-references"
      match="field" use="@ref" />
   ...
   <xsl:template match="/" mode="make-page">
      <html>
         ...
         <body>
            <!--<section>
               <xsl:for-each-group select="//constraint/(.|descendant::require)/(child::* except child::require)"
            group-by="m:path-key((@target,'.')[1])" expand-text="true">
                  <details>
   ...
   <xsl:template
      ...
      <div
         class="{$metaschema-code ! replace(.,'.*-','') }-docs">
         ...
         <xsl:apply-templates select="child::define-assembly | child::define-field"
            ...
            <xsl:with-param name="link-context" tunnel="true"
               select="." />
         </xsl:apply-templates>
      </div>
   </xsl:template>
   ...
   <xsl:variable name="file-map" as="map(xs:string, text())">
      <xsl:map>
         ...
         <xsl:map-entry
            key="'oscal-profile'">profile</xsl:map-entry>
         <xsl:map-entry
            key="'oscal-component-definition'">component</xsl:map-entry>
         <xsl:map-entry
            key="'oscal-ssp'">ssp</xsl:map-entry>
         <xsl:map-entry key="'oscal-poam'">poam</xsl:map-entry>
         <xsl:map-entry
            key="'oscal-ap'">assessment-plan</xsl:map-entry>
         <xsl:map-entry key="'oscal-ar'">
   assessment-results</xsl:map-entry>
      </xsl:map>
   </xsl:variable>
   ...
   <xsl:template
      ...
      <p
         class="description{ ' global'[$is-global]}">
         <!--<span
         class="usa-tag">Description</span>
         <xsl:text> </xsl:text>-->
         <xsl:apply-templates />
      </p>
   </xsl:template>
   ...
   <xsl:template
      ...
      <xsl:variable
         name="nested" select="count(.|ancestor::define-assembly|ancestor::define-field)" />
      <xsl:variable
         name="references"
      ...
      <xsl:variable
         name="singular-names" as="node()*">
         <xsl:variable name="def-single-name" select="(root-name, use-name, @name)[1]" />
         <xsl:for-each
            select="$references[empty(group-as)]">
            <xsl:sequence select="(use-name, $def-single-name, @ref)[1]" />
         </xsl:for-each>
         <xsl:sequence
            select="$def-single-name[empty($references) or exists($references/group-as)]"
         />
      </xsl:variable>
      <!--<xsl:variable
      name="group-names" select="group-as/@name, $references/group-as/@name"/>-->
      <div
   ...
   <xsl:template
      ...
      <xsl:for-each-group select="remarks[not(@class != 'xml')]"
         group-by="true()">
   ...
   <!--<xsl:template
   match="code[. = (/*/@name except ancestor::*/@name)]">
      <a href="#{.}">
         <xsl:next-match/>
      </a>
   </xsl:template>-->
   ...
   <!--<xsl:variable
   name="github-base" as="xs:string">https://github.com/usnistgov/OSCAL/tree/main</xsl:variable>-->

   <xsl:template
      name="report-module" />

   <!--<xsl:template
   name="report-module-really">
   ...
   <xsl:template
      ...
      <style type="text/css" class="doe">
         <xsl:sequence
            select="unparsed-text('../common/hugo-uswds.css', 'utf-8') ! replace(., '#xD;', '')" />
      </style>
   </xsl:template>

   <xsl:template
      mode="occurrence-code" match="*">
      <xsl:param name="require-member" select="false()" />
      <xsl:variable name="minOccurs"
         select="if ($require-member) then  (@min-occurs[not(.='0')], '1')[1]
         else (@min-occurs, '0')[1]" />
      <xsl:variable
         name="maxOccurs"
         select="
   ...
   <xsl:function name="m:has-properties"
      as="xs:boolean">
   ...
   <xsl:template
      ...
      <xsl:variable name="identifier" select="string-join(ancestor-or-self::*/@name,'/')" />
      <!--<xsl:variable
      name="schema-base" select="replace($metaschema-code,'-xml$','')"/>-->
      <div
         class="crosslink">
   ...
   <xsl:template
      match="define-flag" mode="make-definition">
         ...
         <xsl:for-each-group select="key('flag-references', @name)/parent::*" group-by="true()">
            <p><xsl:text>This property appears on: </xsl:text>
               <xsl:for-each-group select="current-group()" group-by="@name">
                  ...
                  <xsl:if
                     test="position() gt 1 and position() eq last()"> and </xsl:if>
                  <xsl:apply-templates
                     select="." mode="link-usage" />
   ...
   <xsl:template
      match="define-field" mode="make-definition">
      <xsl:variable name="showing-flags"
         select="(define-flag | flag)[not(@name=../json-value-key-flag/@flag-ref)]" />
      <section
         class="definition define-field" id="global_{@name}">
         <xsl:call-template name="definition-header">
         ...
         <xsl:if test="exists($value-property-proxy | $showing-flags)">
            <div class="model properties">
               ...
               <ul>
                  ...
                  <xsl:with-param
                        name="make-page-links" tunnel="true" select="false()" />
                  </xsl:apply-templates>
   ...
   <xsl:template
      match="define-assembly" mode="make-definition">
      <section class="definition define-assembly" id="global_{@name}">
         ...
         <xsl:for-each select="root-name">
            <p class="h5"><code xsl:expand-text="true">{ . }</code> is a root (containing) object
   for thi schema.</p>
         </xsl:for-each>
         <xsl:apply-templates mode="appears-in" select="." />
         <xsl:call-template name="remarks-group" />
         <xsl:variable name="for-properties"
            select="flag | define-flag |
   ...
   <xsl:template
      match="*" mode="appears-in" />
      
   <xsl:template match="/METASCHEMA/*" mode="appears-in">
      <xsl:variable name="flag-references" select="key('flag-references', @name)/self::flag" />
      <xsl:variable
         name="field-references" select="key('field-references', @name)/self::field" />
      <xsl:variable
         name="assembly-references" select="key('assembly-references', @name)/self::assembly" />
      <xsl:variable
         name="references"
      ...
      <xsl:call-template
         ...
         <xsl:with-param name="link-to"
            select="($field-references | $assembly-references)[group-as/@in-json='BY_KEY']" />
         <xsl:with-param
            name="statement">This object appears, with any others of its type, grouped as a property
   of </xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="link-paragraph">
         <xsl:with-param name="link-to"
            select="($field-references | $assembly-references)[group-as/@in-json='SINGLETON_OR_ARRAY']" />
         <xsl:with-param
            name="statement">This object appears as a property of, or a member of an array property
   defined for </xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="link-paragraph">
         <xsl:with-param name="link-to"
            select="($field-references | $assembly-references)[ exists( group-as[not(@in-json=('BY_KEY','SINGLETON_OR_ARRAY'))] ) ]" />
         <xsl:with-param
            name="statement" expand-text="true">This object appears as a member of an array property
   defined for </xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="link-paragraph">
         <xsl:with-param name="link-to"
         ...
         <xsl:with-param
            name="statement">A property of this name is also defined for </xsl:with-param>
      </xsl:call-template>
   </xsl:template>
   ...
   <!--<xsl:template
   match="*[use-name|root-name]/@name | *[use-name]/@ref">
      <code>
   ...
   <xsl:template
      match="define-flag/@name | flag/@name">
      <code>
         <xsl:value-of select="." />
      </code>
   </xsl:template>
   ...
   <xsl:template
      match="field | define-field | assembly | define-assembly">
      <li class="model-entry">
         <xsl:call-template name="model-description" />
      </li>
   </xsl:template>
   ...
   <!--<xsl:template
   match="define-field[@as-type='markup-multiline'][@in-xml='UNWRAPPED']">
      <li class="model-entry">
         <p>An optional sequence of prose (markup) elements including <code>p</code>, lists, tables and
   headers (<code>h1-h6</code>).</p>
         <xsl:call-template name="model-description"/>
      </li>
   </xsl:template>-->

   <xsl:template
      match="*" mode="requirement"> [0 or 1]</xsl:template>

   <xsl:template
      match="*[exists(@required)]" mode="requirement"> [1]</xsl:template>
   ...
   <xsl:template
      match="*" mode="model-description" expand-text="true">
      <xsl:message>UNEXPECTED MATCH ON { local-name() }</xsl:message>
   </xsl:template>

   <xsl:template
      match="*[group-as]" mode="model-description" priority="100">
   ...
   <xsl:template match="*" mode="group-header"
      expand-text="true">
   ...
   <xsl:template
      match="*" mode="group-type">array</xsl:template>

   <xsl:template match="*[@in-json = 'BY_KEY']"
      mode="group-type">object</xsl:template>

   <xsl:template
      match="*[@in-json = 'SINGLETON_OR_ARRAY']" mode="group-type">object (when a singleton) or
   array (when multiple)</xsl:template>
   ...
   <xsl:template
      match="*[@in-json = 'BY_KEY']" mode="group-type" expand-text="true">object's properties have
   keys bound to the <code>{ group-as/@json-key-flag }</code></xsl:template>

   <xsl:template
      match="*[@in-json = 'SINGLETON_OR_ARRAY']" mode="group-type">object (when a singleton) or
   array (when multiple)</xsl:template>

   <xsl:template match="*[exists(@ref)]"
      mode="model-description" expand-text="true">
      <xsl:param name="link-context" tunnel="true" required="yes" />
      <!--<xsl:param
      name="link-context" tunnel="true" select="ancestor::model[1]/parent::define-assembly |
      parent::define-field | parent::define-assembly)"/>-->
      <xsl:variable name="grouped"
         select="exists(group-as)" />
      <xsl:variable name="definition"
         select="key('assembly-definitions', self::assembly/@ref) |
         key('field-definitions', self::field/@ref) | key('flag-definitions', self::flag/@ref)" />
      <xsl:variable
         name="is-a-flag" select="exists(self::flag)" />
      <!--<xsl:variable
      name="too-deep"
         select="exists(ancestor::model[2] | (parent::define-field |
      parent::define-assembly)/ancestor::model)"/>-->
      <xsl:if test="empty($definition)">
         <xsl:message>NO DEFINITION FOUND FOR { local-name() }</xsl:message>
      </xsl:if>
      <div
         class="model-descr" tabindex="0">
         <div class="model-summary">
            <p class="modeling usename">
   ...
   <xsl:template mode="key-name"
      match="*[group-as/@in-json = 'ARRAY']">(array member)</xsl:template>

   <xsl:template
      mode="key-name" match="*[group-as/@in-json = 'BY_KEY']" expand-text="true">{{{
   group-as/@json-key-flag }}}</xsl:template>

   <xsl:template mode="key-name"
      match="*[group-as/@in-json = 'SINGLETON_OR_ARRAY']">(array member or object)</xsl:template>

   <xsl:template
      match="define-assembly | define-field | define-flag" mode="model-description"
      expand-text="true">
      ...
      <div class="model-descr" tabindex="0"
         ...
         <!-- <xsl:if test="exists($proxy-of)">
            <xsl:message expand-text="true">seeing proxy-of: { local-name($proxy-of) } { $proxy-of/@name
         }</xsl:message>
         </xsl:if>-->
         <xsl:call-template name="display-applicable-constraints">
   ...
   <xsl:template
      ...
      <xsl:variable name="for-properties"
         ...
         model/(.|choice)/( define-field | define-assembly | field | assembly )" />
      <xsl:for-each-group
         select="$for-properties" group-by="true()" expand-text="true">
            ...
            <details>
               <summary class="subhead">
                  ...
                  <xsl:for-each select="current-group()">
                     ...
                     <code>{
   (m:use-name(.),@name,@ref,local-name())[1] }</code>
                  </xsl:for-each>
               </summary>
               <ul>
                  <xsl:apply-templates select="current-group()">
      ...
      <!--<xsl:for-each-group
      select="$definition/(define-flag | flag)" group-by="true()" expand-text="true">
         <div class="attributes">
            <details open="open">
               <summary  class="subhead">
                  <xsl:text>{ if (count(current-group()) eq 1) then 'Attribute' else 'Attributes' } ({
      count(current-group()) }): </xsl:text>
                  <xsl:for-each select="current-group()">
                     <xsl:if test="not(position() eq 1)">, </xsl:if>
   ...
   <xsl:template
      match="flag | define-flag | define-field | define-assembly"
      mode="make-link-to-global" />

   <xsl:template match="*" mode="make-link-to-global"
      expand-text="true">
      <div class="deflink">
         <!-- XXX -->
         <a href="#xxx">Link to <code>{ m:use-name(.) }</code> global definition to see contents.</a>
      </div>
   </xsl:template>
   ...
   <xsl:template
      match="constraint" expand-text="true">
   ...
   <xsl:template
      ...
      <xsl:if
         test="count($what) gt 1"> and </xsl:if>
      <code>
   ...
   <xsl:template
      priority="3" match="constraint//require">
      <xsl:apply-templates />
   </xsl:template>
   ...
   <!--<xsl:function
   name="m:constraint-key" as="xs:string">
      <xsl:param name="who" as="element()"/>
      <xsl:text>boo</xsl:text>
   </xsl:function>-->

   <xsl:template
      match="*" mode="report-context" expand-text="true">
      <code>
   ...
   <xsl:template
      match="*" mode="report-target">
      <xsl:text>/</xsl:text>
      <xsl:value-of select="@target" />
   </xsl:template>

   <xsl:template
      priority="3" mode="report-target" match="has-cardinality" />
   ...
   <xsl:template priority="2"
      match="allowed-values" expand-text="true">
   ...
   <xsl:template
      priority="2" match="matches[@regex]" expand-text="true">
   ...
   <xsl:template
      priority="2" match="matches[@datatype]" expand-text="true">
      <div class="constraint">
         <p>
            <span class="usa-tag">constraint</span>
            <xsl:apply-templates select="." mode="report-context" />
   ...
   <xsl:template
      priority="2" match="has-cardinality" expand-text="true">
      <div class="constraint">
         <p>
            <span class="usa-tag">constraint</span>
   ...
   @target }</code>
            ...
            <b>{
   (@max-occurs,'unbounded')[1]}</b>.</p>
      </div>
   </xsl:template>

   <xsl:template priority="2"
      match="index-has-key" expand-text="true">
      <xsl:variable name="target" select="@target[not(. = ('.', 'value()'))]" />
      <div
         class="constraint">
         <p>
   ...
   <xsl:template
      priority="2" match="index" expand-text="true">
      <div class="constraint">
         <p>
            <span class="usa-tag">constraint</span>
            <xsl:apply-templates select="." mode="report-context" />
   ...
   <xsl:template
      ...
      <!-- debug <xsl:if test="@name='property'">
         <xsl:message expand-text="true">{ local-name(.) } { m:use-name(.) } {
      count($applicable-constraints) }</xsl:message>
         <xsl:for-each select="$applicable-constraints/self::allowed-values">
            <xsl:message expand-text="true">{ serialize(.) }</xsl:message>
         </xsl:for-each>
      </xsl:if>-->
      <xsl:for-each-group
         select="$applicable-constraints/self::allowed-values" group-by="true()"
         expand-text="true">
         <div class="constraints">
            <xsl:apply-templates select="current-group()" />
            <!--<xsl:for-each-group
            select="$applicable-constraints except $applicable-constraints/self::allowed-values"
            group-by="true()">
               <details>
                  <summary class="subhead">Applicable { if (count(current-group()) eq 1) then 'constraint' else
            'constraints' } ({ count(current-group()) })</summary>
                  <xsl:apply-templates select="current-group()"/>
               </details>
            </xsl:for-each-group>-->
         </div>
   ...
   <!-- key 'constraints-for-target' is too greedy: for a reference appearing in a local definition,
     the definition context can exclude constraints that are defined to apply to the node in other
   contexts.
     This filter returns 'true' pairwise for any constraint and context-indicating element (whether
   definition or reference),
     when the two overlap. This must account for when constraints designate multiple targets. -->
   <xsl:function
      name="m:include-constraint" as="xs:boolean">
      ...
      <xsl:variable
         name="constraint-context-path"
         select="string-join($constraint/ancestor-or-self::*/m:use-name(.), '/')" />
      <xsl:variable
         name="constraint-targets" as="xs:string*">
         <xsl:choose>
      ...
      <!-- debugging <xsl:if test="contains($context-path,'control')">
         <xsl:message expand-text="true">on { name($constraint)}, testing { $context-path } against {
      string-join($constraint-targets,', ')} getting {
      m:match-paths($constraint-targets[1],$context-path)}</xsl:message>
      </xsl:if>-->
      <xsl:sequence
         select="
   ...
   <xsl:template
      mode="make-definition" match="field | flag | assembly" />
   ...
   <xsl:template
      match="remarks[@class != 'json']" priority="2" />

   <xsl:template
      match="remarks[@class = 'json']/p[1]">
      <p class="p">
   ...
   <xsl:template
      match="m:*" xmlns:m="http://csrc.nist.gov/ns/oscal/metaschema/1.0"
      mode="as-example" />

   <xsl:template
      match="define-flag | define-field[empty(flag | define-flag)]"
   ...
   <xsl:template
      match="define-field" mode="representation-in-json">
      <xsl:apply-templates select="." mode="type-annotation" />
   </xsl:template>

   <xsl:template
      match="*" mode="get-field-value-name">STRVALUE</xsl:template>
   
   <xsl:template
      match="*[@as-type='markup-multiline']" mode="get-field-value-name">RICHTEXT</xsl:template>
   
   <xsl:template
      match="*[exists(json-value-key)]" mode="get-field-value-name">
      <xsl:value-of select="json-value-key" />
   </xsl:template>
   
   <xsl:template priority="2"
      match="*[exists(json-value-key-flag/@flag-ref)]" mode="get-field-value-name">
   ...
   <xsl:template
      match="*" mode="get-field-value-description">
      <m:description>This property provides the (nominal) value for this object as a whole.</m:description>
   </xsl:template>
   
   <xsl:template
      priority="2" match="*[exists(json-value-key-flag/@flag-ref)]"
      mode="get-field-value-description">
      <m:description>A property whose name is distinct from assigned properties for this object is
   taken as its (nominal) value, while its key is taken to be the value of the <code>
            <xsl:value-of select="json-value-key-flag/@flag-ref" />
         </code> property.</m:description>
   </xsl:template>
   
   <xsl:template
      match="*" mode="type-annotation" />
      
   <xsl:template match="*[@as-type = 'markup-multiline']"
      mode="type-annotation">
      <p>As such, this value permits expression of marked up text in Markdown format, according to
   the rules described for the (text-based) datatype. This datatype permits the expression of <em>
   block-level</em> constructs including paragraphs, lists and simple tables, potentially including
   simple formatting such as bold or typographic emphasis. This representation is designed for the
   relatively unconstrained capture of simple <q>free
   ...
   uncontrolled semantic encoding not subject to detection, regularization or validation.</p>
      <p>This
   data construct is designed to be minimalistic for purposes of ease of development and
   interchange. It will not fit all operational scenarios; when <xsl:apply-templates select="."
            mode="metaschema-type" /> is not adequate for purposes of necessary (informational)
   fidelity to information encoded in source formats (and subsequently converted into OSCAL),
   alternative strategies are available for such data capture. Users and stakeholders who expose
   requirements in this area are encouraged to provide feedback and request guidance.</p>
   </xsl:template>
   
   <xsl:template
      match="*[@as-type = 'markup-line']" mode="type-annotation">
      <p>As such, this value permits expression of marked up text in Markdown format, according to
   the rules described for the (text-based) datatype. This datatype permits the expression of <em>
   inline</em> constructs, including text marked as strong, bold or emphatic.</p>
      <p>This data
   construct is designed to be minimalistic for purposes of ease of development and interchange. It
   will not fit all operational scenarios; when <xsl:apply-templates select="."
            mode="metaschema-type" /> is not adequate for purposes of necessary (informational)
   fidelity to information encoded in source formats (and subsequently converted into OSCAL),
   alternative strategies are available for such data capture. Users and stakeholders who expose
   requirements in this area are encouraged to provide feedback and request guidance.</p>
   ...
   <xsl:template
      mode="metaschema-type json-type" match="define-assembly | define-field"
   >object</xsl:template>
   
   <xsl:template
      mode="json-type" match="*[exists(@ref)]">
      <xsl:variable name="definition"
         select="key('assembly-definitions', self::assembly/@ref,$home) |
         key('field-definitions', self::field/@ref,$home) | key('flag-definitions', self::flag/@ref,$home)" />
      <xsl:apply-templates
         mode="#current" select="$definition" />
   </xsl:template>
   
   <xsl:template mode="json-type"
      match="*">object</xsl:template>


   <!--<xsl:variable
   name="definition" select="key('assembly-definitions', self::assembly/@ref) |
      key('field-definitions', self::field/@ref) | key('flag-definitions', self::flag/@ref)"/>
   -->
   ...
   <xsl:template
      mode="metaschema-type" match="METASCHEMA/define-assembly">
      <!-- XXX -->
      <a href="#xxx">object
   (globally&#xA0;defined)</a>
   </xsl:template>
   ...
   <xsl:template match="*"
      mode="constraint-context">
      <summary>In <xsl:apply-templates
            select="ancestor::define-assembly | ancestor::define-field | ancestor::define-flag"
            mode="read-context" />
         <xsl:apply-templates select="ancestor::require"
            mode="read-context" />
      </summary>
   </xsl:template>

   <xsl:template
      match="define-assembly | define-field" mode="read-context">
      <xsl:if test="position() gt 1">/</xsl:if>
      <xsl:apply-templates select="@name" />
   </xsl:template>

   <xsl:template
      match="define-flag" mode="read-context">
      <xsl:if test="position() gt 1">/@</xsl:if>
      <xsl:apply-templates select="@name" />
   </xsl:template>

   <xsl:template
      match="require" mode="read-context">
   ...
   <xsl:function
      name="m:use-name" as="xs:string?">
      <xsl:param name="who" as="element()" />
      <xsl:variable name="definition"
         select="$who/self::assembly/key('assembly-definitions', @ref) |
      ...
      <!--<xsl:if
      test="$who/self::field/@ref='all'">
         <xsl:message expand-text="true">seeing 'all'; m:use-name is { $using-name }</xsl:message>
      </xsl:if>
      <xsl:if test="empty($using-name)">
         <xsl:message expand-text="true">not seeing a usable name on { local-name($who) || ' ' ||
      $who/(@name|@ref)}</xsl:message>
      </xsl:if>-->

      <!--<xsl:if
      test="exists($who/(self::field|self::assembly|self::flag)) and empty($definition">
      ...
      <xsl:if test="empty(
      $who/(self::define-assembly|self::define-field|self::define-flag|self::assembly|self::field|self::flag)/
         (root-name, key('definitions',@ref)/root-name, @name, @ref)[1] )">
