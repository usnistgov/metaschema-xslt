...
<xsl:stylesheet version="3.0"
   ...
   <xsl:param
      name="xml-reference-page">xml/reference</xsl:param>
   <xsl:param name="json-map-page">
   json/outline</xsl:param>
   ...
   <xsl:variable
      name="xml-reference-link" select="$path-to-common || $xml-reference-page" />
   <xsl:variable
      name="json-map-link" select="$path-to-common || $json-map-page" />
   ...
   <!-- Map traversal -->
   <xsl:template match="*[exists(@key)]"
      expand-text="true">
      <xsl:variable name="level" select="count(ancestor-or-self::*[exists(@key)])" />
      <xsl:variable
         name="header-tag" select="if ($level le 6) then ('h' || $level) else 'p'" />
      <xsl:variable
         name="grouped-object" select="(self::array | self::singleton-or-array)/*" />
      <xsl:variable
         name="me" select="($grouped-object,.)[1]" />

      <div
         class="model-entry definition { tokenize($me/@_metaschema-json-id,'/')[2] }">
   ...
   'definition' else 'instance' }-header</xsl:variable>
         <div class="{ $header-class }">
            ...
            <xsl:element name="a" namespace="http://www.w3.org/1999/xhtml">
               ...
               <xsl:attribute
                  name="title">Focus on {@key} details</xsl:attribute>
   ...
   toc{$level} name </xsl:attribute>
         ...
         <xsl:where-populated>
            <div class="body">
               ...
               <!--<xsl:variable
               name="mine" select="$me / (array | singleton-or-array | object | string | number |
               boolean)"/>-->
               ...
               <xsl:for-each-group select="$mine" group-by="true()">
                  <!--<xsl:message
                  expand-text="true">$mine includes { $mine/name() }</xsl:message>-->
   ...
   <xsl:template
      match="singleton-or-array/*" mode="array-header">
      <div class="array-header">
      ...
      <xsl:apply-templates
         select="description" mode="produce" />
      <xsl:call-template name="remarks-group" />
   </xsl:template>
   ...
   <xsl:template
      ...
      <xsl:if test="@scope='global'">
         ...
         <a class="definition-link"
            href="{$path-to-common || $json-definitions-page }#{ @_metaschema-json-id }">(global
   definition)</a>
      </xsl:if>
   </xsl:template>
