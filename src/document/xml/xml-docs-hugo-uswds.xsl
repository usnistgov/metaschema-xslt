...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
      ...
      <xsl:if test="exists($flags)">
         <xsl:variable name="modal">
            <xsl:choose>
               <xsl:when test="
                     every $f in ($flags)
                        satisfies $f/@required = 'yes'"> must</xsl:when>
   ...
   <xsl:template name="appears-in">
      ...
      <xsl:call-template name="link-paragraph">
         ...
         <xsl:with-param name="statement">An attribute of this name is also defined for
         </xsl:with-param>
      </xsl:call-template>
      <xsl:call-template name="link-paragraph">
         <xsl:with-param name="link-to"
            select="$homonyms/self::define-field | $homonyms/self::define-assembly"/>
         <xsl:with-param name="statement">An element of this name is also defined for
         </xsl:with-param>
      </xsl:call-template>
   </xsl:template>
   ...
   <xsl:template match="model">
      <div class="model">
         <p class="subhead">Contents<xsl:if test="count(*) > 1"> (in order)</xsl:if></p>
         <ul>
            <xsl:apply-templates>
               <xsl:with-param name="make-page-links" tunnel="true" select="false()"/>
            </xsl:apply-templates>
            ...
            <xsl:otherwise>
               <details>
                  <summary>Model from <span class="OM-name">
                        <xsl:value-of select="m:use-name(.)"/>
                     </span> global definition</summary>
                  <xsl:apply-templates mode="make-link-to-global" select="."/>
   ...
   <xsl:template match="define-assembly/model" mode="make-contents">
      ...
      <details open="open">
         <summary class="subhead">Contents<xsl:if test="count(*) > 1"> (in order)</xsl:if></summary>
         <ul>
            <xsl:apply-templates>
               <xsl:with-param name="make-page-links" tunnel="true" select="false()"/>
            </xsl:apply-templates>
   ...
   <xsl:template match="define-field" mode="representation-in-xml">
      ...
      <xsl:if test="normalize-space($my-type) = 'markup-multiline'">
         <p>As such, this element permits an <em>block-level HTML markup</em> subset, syntactically
            well-formed as XML (i.e. following XML syntax rules), including HTML elements
               <code>p</code>, <code> ul</code>, <code>ol</code> and others, along with a simple
            subset of inline <q>face</q> markup such as bold and italics. This format is designed
            for the relatively unconstrained capture of simple <q>free text</q>, i.e. without
            formatting or <q>decoration</q> that might serve as ad-hoc and uncontrolled semantic
            encoding not subject to detection, regularization or validation.</p>
         <p>This data construct is designed to be minimalistic for purposes of ease of development
            and interchange. It will not fit all operational scenarios; when <xsl:apply-templates
      ...
      <xsl:where-populated>
         <details>
            <summary>Constraints that may apply to the data value of this element (in
               context):</summary>
            <xsl:call-template name="display-applicable-constraints"/>
         </details>
      </xsl:where-populated>
   ...
   <xsl:template mode="metaschema-type" match="METASCHEMA/define-assembly">
      <a href="#global_{@name}">element (globally&#xA0;defined)</a>
   </xsl:template>
