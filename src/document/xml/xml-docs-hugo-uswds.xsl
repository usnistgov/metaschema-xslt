...
<xsl:stylesheet version="3.0"
   ...
   <xsl:param name="json-definitions-page"
      as="xs:string">../../json/definitions</xsl:param>
   
   <xsl:variable name="home"
      select="/METASCHEMA" />
   
   <xsl:variable name="all-references"
      select="//flag/@name | //model//*/@ref" />
   
   <xsl:key name="assembly-definitions"
      match="/METASCHEMA/define-assembly" use="@name" />
   <xsl:key name="field-definitions"
      match="/METASCHEMA/define-field" use="@name" />
   <xsl:key name="flag-definitions"
      match="/METASCHEMA/define-flag" use="@name" />
   
   <xsl:key name="flag-references" match="flag"
      use="@ref" />
   ...
   <xsl:template
      ...
      <div
         ...
         <xsl:apply-templates select="child::define-assembly | child::define-field"
            ...
            <xsl:with-param name="make-page-links" tunnel="true"
               select="true()" />
         </xsl:apply-templates>
      </div>
   </xsl:template>
   ...
   <xsl:variable
      name="file-map" as="map(xs:string, text())">
      <xsl:map>
   ...
   <xsl:template match="*" mode="link-here">
      ...
      <a
         href="#local_{string-join( (ancestor::*|.)/(@name|@ref),'-')}" class="name-link">
         <xsl:value-of select="m:use-name(.)" />
      </a>
   </xsl:template>
   
   <xsl:template
      match="METASCHEMA/define-assembly | METASCHEMA/define-field" mode="link-here">
   ...
   <xsl:template
      ...
      <div
         ...
         <xsl:element name="h{ $nested }" namespace="http://www.w3.org/1999/xhtml"
            ...
            <!--<xsl:for-each-group
            select="$using-names" group-by="string(.)">
   ...
   <xsl:template
      match="example[empty(* except (description | remarks))]" />
   ...
   <xsl:template
      ...
      <xsl:variable name="maxOccurs"
         select="(@max-occurs,'1')[1] ! (if (. eq 'unbounded') then '&#x221e;' else .)" />
      <span
         class="cardinality">
         <xsl:text>[</xsl:text>
         <xsl:choose>
   ...
   must</xsl:when>
         ...
         <div
            class="model">
            <p xsl:expand-text="true">The <span class="OM-name">{ m:use-name(.) }</span> element {
   $modal } have { $noun }:</p>
            <ul>
   ...
   <xsl:template
      ...
      <section class="definition define-assembly" id="global_{@name}">
         ...
         <xsl:for-each select="root-name">
            <p class="h5"><code xsl:expand-text="true">{ . }</code> is a root (containing) element
   in this schema. </p>
         </xsl:for-each>
         <xsl:call-template name="appears-in" />
         <xsl:call-template name="remarks-group" />
         <xsl:for-each-group select="define-flag | flag" group-by="true()" expand-text="true">
            <div class="model attributes">
               <p class="subhead">{ if (count(current-group()) eq 1) then 'Attribute' else
   'Attributes' } ({ count(current-group()) })</p>
               <ul>
                  <xsl:apply-templates select="current-group()">
   ...
   <xsl:template
      ...
      <xsl:variable
         name="field-references" select="key('field-references', @name)" />
      <xsl:variable
         name="assembly-references" select="key('assembly-references', @name)" />
      <xsl:variable
         name="references" select="$flag-references | $field-references | $assembly-references" />
      <xsl:variable
         name="homonyms" select="key('using-name', m:use-name(.)) except (. | $references)" />
      <xsl:call-template
         name="link-paragraph">
         <xsl:with-param name="link-to" select="$field-references | $assembly-references" />
         <xsl:with-param
            name="statement">This element appears inside </xsl:with-param>
      </xsl:call-template>
      <xsl:call-template
         name="link-paragraph">
         <xsl:with-param name="link-to" select="$flag-references" />
         <xsl:with-param name="statement">An
   attribute of this name is also defined for </xsl:with-param>
      </xsl:call-template>
      <xsl:call-template
         name="link-paragraph">
         <xsl:with-param name="link-to"
            select="$homonyms/self::define-field | $homonyms/self::define-assembly" />
         <xsl:with-param
            name="statement">An element of this name is also defined for </xsl:with-param>
      </xsl:call-template>
   </xsl:template>
   ...
   <xsl:template
      match="*" mode="requirement"> [optional]</xsl:template>

   <xsl:template
      match="*[exists(@required)]" mode="requirement"> [required]</xsl:template>

   <xsl:template
      match="model[not(*)]" priority="3" />
   ...
   <xsl:template
      match="*[exists(@ref)]" mode="model-description" expand-text="true">
      <xsl:param name="make-page-links" tunnel="true" select="true()" />
      <xsl:param
         name="make-model-links" tunnel="true" select="true()" />
      ...
      <xsl:variable name="too-deep"
         select="exists(ancestor::model[2])" />
      ...
      <div
         ...
         <div class="model-summary">
            ...
            <!--<h3
            class="usename{ ' toc2'[$make-model-links and not($is-a-flag) and not($too-deep)] }">
               <xsl:if test="$make-model-links">
                  <xsl:attribute name="id" select="'#local_' || string-join(ancestor-or-self::*/(@name|@ref), '-') ||
            '_ref-h3'"/>
               </xsl:if>
               <xsl:value-of select="m:use-name(.)"/>
            </h3>-->
            <!--<h3
            class="usename{ ' toc2'[not($is-a-flag) and not($too-deep)] }" id="#local_{string-join(
            ancestor-or-self::*/(@name|@ref),'-')}_ref-h3">{ m:use-name(.) }</h3>-->
            <!--<h3
            class="usename{ ' toc2'[not($is-a-flag) and not($too-deep)] }">{ m:use-name(.) }</h3>-->
            <span class="mtyp">
         ...
         <xsl:choose>
            ...
            <xsl:otherwise>
               <details>
                  <summary>Model from <span class="OM-name">
                        <xsl:value-of select="m:use-name(.)" />
                     </span> global
   definition</summary>
                  <xsl:apply-templates mode="make-link-to-global" select="." />
   ...
   <xsl:template
      ...
      <xsl:variable
         name="too-deep" select="exists(ancestor::model[2])" />
      <xsl:variable name="is-a-flag"
         select="exists(self::define-flag)" />
      <xsl:variable name="is-unwrapped"
         select="@in-xml='UNWRAPPED'" />
      
      <div class="model-descr" tabindex="0">
         <xsl:if test="$make-model-links">
            <xsl:attribute name="id"
               select="'#local_' || string-join( ancestor-or-self::*/@name,'-')" />
         </xsl:if>
   ...
   <xsl:template
      match="flag | define-flag | define-field | define-assembly" mode="make-link-to-global" />
   
   <xsl:template
      match="*" mode="make-link-to-global" expand-text="true">
      <div class="deflink">
   ...
   <xsl:template
      match="*[exists(@ref)]" mode="make-contents">
      <xsl:variable name="definition"
         select="key('assembly-definitions', self::assembly/@ref) |
         key('field-definitions', self::field/@ref) | key('flag-definitions', self::flag/@ref)" />
      <xsl:apply-templates
   ...
   <xsl:template
      match="define-assembly" mode="make-contents" expand-text="true">
      <xsl:apply-templates select="model" mode="#current" />
   </xsl:template>
   
   <xsl:template
      match="define-assembly/model" mode="make-contents">
   ...
   <xsl:template priority="2"
      mode="report-target" match="*[empty(@target) or @target=('.','value()')]" />
   
   <xsl:template
      priority="2" match="allowed-values" expand-text="true">
   ...
   <!-- This filter returns 'true' pairwise for any constraint and context-indicating element
   (whether definition or reference),
   ...
   <xsl:template
      match="remarks[@class != 'xml']" priority="2" />

   <xsl:template
      match="remarks[@class = 'xml']/p[1]">
      <p>
   ...
   <xsl:template
      match="text()[not(matches(.,'\S'))]" mode="serialize">
   ...
   <xsl:template
      match="define-flag" mode="representation-in-xml">
      <p>An attribute <xsl:apply-templates select="." mode="metaschema-type" />.</p>
   </xsl:template>

   <xsl:template
      match="define-field" mode="representation-in-xml">
      <xsl:variable name="unwrapped-occurrences"
         select=".[@in-xml='UNWRAPPED'] | key('field-references',@name)[@in-xml='UNWRAPPED']" />
      <xsl:variable
         name="my-type">
         <xsl:apply-templates select="." mode="metaschema-type" />
      </xsl:variable>
      <p>An element
   with a value of type <xsl:sequence select="$my-type" />.</p>
      <xsl:if
         test="normalize-space($my-type)='markup-multiline'">
         <p>As such, this element permits an <em>block-level HTML markup</em> subset, syntactically
   well-formed as XML (i.e. following XML syntax rules), including HTML elements <code>p</code>, <code>
   ul</code>, <code>ol</code> and others, along with a simple subset of inline <q>face</q> markup
   such as bold and italics. This format is designed for the relatively unconstrained capture of
   simple <q>free text</q>, i.e. without formatting or <q>decoration</q> that might serve as ad-hoc
   and uncontrolled semantic encoding not subject to detection, regularization or validation.</p>
      <p>This
   ...
   requirements in this area are encouraged to provide feedback and request guidance.</p>
      ...
      <xsl:if
         test="exists($unwrapped-occurrences)">
         <p>
            <xsl:text>When appearing in </xsl:text>
            <xsl:for-each
               select="distinct-values($unwrapped-occurrences/ancestor::model/m:use-name(..))">
               <xsl:if test="position() gt 1 and last() ne 2">, </xsl:if>
               <xsl:if
                  test="position() gt 1 and position() eq last()"> or </xsl:if>
               <a href="#{ . }"
                  xsl:expand-text="true">{ . }</a>
            </xsl:for-each>
   ...
   <xsl:template
      mode="metaschema-type" match="define-field">
      <xsl:variable name="given-type"
   ...
   <xsl:template
      mode="metaschema-type" match="define-assembly">element</xsl:template>
   
   <xsl:template
      mode="metaschema-type" match="METASCHEMA/define-assembly"><a href="#global_{@name}">element
   (globally&#xA0;defined)</a></xsl:template>
