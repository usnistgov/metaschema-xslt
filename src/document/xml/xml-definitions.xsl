...
<xsl:stylesheet version="3.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template mode="definition-title-text" expand-text="true"
        match="model//define-field[@as-type = 'markup-multiline'][@in-xml = 'UNWRAPPED']"
        >(unwrapped)</xsl:template>
    ...
    <xsl:template mode="definition-title-text" expand-text="true"
        ...
        <xsl:choose>
            <xsl:when
                test="(. | $definition)/@as-type = 'markup-multiline' and @in-xml = 'UNWRAPPED'">
                (unwrapped)</xsl:when>
            <xsl:otherwise>
                <xsl:next-match/>
            </xsl:otherwise>
    ...
    <xsl:template match="group-as" expand-text="true">
        <xsl:if test="@in-xml = 'GROUPED'">
            <p><span class="usa-tag">wrapper element</span>&#xA0;<code class="name">{ @name
                }</code></p>
        </xsl:if>
    </xsl:template>
