...
<xsl:stylesheet version="3.0"
    ...
    <xsl:param name="xml-definitions-page">
    xml/definitions</xsl:param>
    <xsl:param name="json-definitions-page">json/definitions</xsl:param>
    
    
    <xsl:template
        match="METASCHEMA/json-base-uri" />
    ...
    <xsl:template
        mode="definition-title-text" expand-text="true"
    ...
    <xsl:template
        ...
        <xsl:choose>
            <xsl:when test="(.|$definition)/@as-type='markup-multiline' and @in-xml='UNWRAPPED'">
    (unwrapped)</xsl:when>
            <xsl:otherwise>
                <xsl:next-match />
            </xsl:otherwise>
    ...
    <xsl:template
        ...
        <xsl:for-each-group
            select="flag | define-flag" group-by="true()" expand-text="true">
            <details open="open">
                <summary>{ if (count(current-group()) ne 1) then 'Attributes' else 'Attribute' } ({
    count(current-group()) }):</summary>
        ...
        <xsl:for-each-group
            select="model/*" group-by="true()" expand-text="true">
            <details open="open">
    ...
    <xsl:variable
        name="json-definitions-link" select="$path-to-common || $json-definitions-page" />

    <!-- JSON values dropped in default traversal for XML docs   -->
    <xsl:template
        match="json-value-key | json-key | json-key-flag" />
    ...
    <xsl:template
        name="crosslink">
        <div class="crosslink">
            <a class="usa-button" href="{$json-definitions-link}#{@_metaschema-json-id}">Switch to
    JSON</a>
        </div>
    </xsl:template>
    ...
    <xsl:template
        ...
        <xsl:if
            ...
            <xsl:text> </xsl:text>
            <a class="definition-link" href="#{ @_metaschema-xml-id }">(global
    definition)</a>
        </xsl:if>
    </xsl:template>
