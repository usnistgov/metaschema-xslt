...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <xsl:template name="remarks-group">
      ...
      <xsl:for-each-group select="$these-remarks[not(contains-token(@class, 'json'))]"
         group-by="true()">
   ...
   <xsl:template match="*[exists(@gi)]" expand-text="true">
      ...
      <xsl:variable name="header-tag" select="
            if ($level le 6) then
               ('h' || $level)
            else
               'p'"/>
      <div class="model-entry definition { tokenize(@_metaschema-xml-id,'/')[2] }">
         ...
         <div class="{ $header-class }">
            ...
            <xsl:element name="a" namespace="http://www.w3.org/1999/xhtml">
               ...
               <xsl:element expand-text="true" name="{ $header-tag }"
                  namespace="http://www.w3.org/1999/xhtml">
                  <xsl:attribute name="id" select="@_tree-xml-id"/>
                  <xsl:attribute name="class">reference-element-anchor toc{ $level}
                     name</xsl:attribute>
