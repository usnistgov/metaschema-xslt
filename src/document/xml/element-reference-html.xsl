...
<xsl:stylesheet version="3.0"
   ...
   <xsl:param
      name="json-reference-page">json/reference</xsl:param>
   <xsl:param name="xml-map-page">
   xml/outline</xsl:param>
   <xsl:param name="xml-definitions-page">xml/definitions</xsl:param>
   
   <xsl:variable
      name="datatype-page" as="xs:string">/reference/datatypes</xsl:variable>
   
   <xsl:template
      match="metadata/json-base-uri" />
      
   <xsl:template name="remarks-group">
      <xsl:param name="these-remarks" select="remarks" />
      <xsl:for-each-group
         select="$these-remarks[not(contains-token(@class,'json'))]" group-by="true()">
   ...
   <xsl:variable
      name="json-reference-link" select="$path-to-common || $json-reference-page" />
   <xsl:variable
      name="xml-map-link" select="$path-to-common || $xml-map-page" />
   ...
   <xsl:template match="*[exists(@gi)]"
      ...
      <xsl:variable
         name="header-tag" select="if ($level le 6) then ('h' || $level) else 'p'" />
      <div
         class="model-entry definition { tokenize(@_metaschema-xml-id,'/')[2] }">
   ...
   'definition' else 'instance' }-header</xsl:variable>
         <div class="{ $header-class }">
            ...
            <xsl:element name="a" namespace="http://www.w3.org/1999/xhtml">
               ...
               <xsl:attribute
                  name="class">reference-element-anchor toc{ $level } name </xsl:attribute>
               <xsl:attribute
                  name="title">Focus on {@gi} details</xsl:attribute>

               <xsl:element expand-text="true"
                  name="{ $header-tag }" namespace="http://www.w3.org/1999/xhtml">
                  <xsl:attribute name="id" select="@_tree-xml-id" />
                  <xsl:attribute name="class">reference-element-anchor
   toc{ $level} name</xsl:attribute>
                  <xsl:text>{ @gi }</xsl:text>
               </xsl:element>

            </xsl:element>

            <!-- Only assign the @id here if there is a value and the type is not markup-multiline,
            since in the other cases it
         ...
         <xsl:where-populated>
            <div class="body">
               ...
               <xsl:for-each-group
                  select="element | choice[exists(child::element)]  | value[@as-type='markup-multiline']"
                  group-by="true()">
                  <xsl:variable name="elements"
                     select="current-group()/ (self::element | self::choice/child::element)" />
                  <details
                     class="properties elements" open="open">
                     <summary>
   ...
   <xsl:template
      match="value[empty(@gi)][@as-type='markup-multiline']" expand-text="true">
      <xsl:variable name="level" select="count(ancestor-or-self::*[exists(@gi)])" />
      <xsl:variable
         name="header-tag" select="if ($level le 6) then ('h' || $level) else 'p'" />
      <div
         ...
         <div class="{ $header-class }">
            ...
            <xsl:element expand-text="true" name="a" namespace="http://www.w3.org/1999/xhtml">
               ...
               <xsl:attribute
                  name="class">reference-element-anchor toc{ $level} name</xsl:attribute>
               <xsl:attribute
                  name="title">Get {@_tree-xml-id} details</xsl:attribute>

               <!-- ===!!! The Headers 1-6 that are being wrapped around !!!=== --> 
               <xsl:element
                  expand-text="true" name="{ $header-tag }" namespace="http://www.w3.org/1999/xhtml">
                  <xsl:attribute name="id" select="@_tree-xml-id" />
                  <xsl:attribute name="class">toc{
   $level} name </xsl:attribute>
            ...
            <!--<xsl:call-template
            name="crosslink-to-json"/>-->
            <!--<xsl:apply-templates
            select="formal-name" mode="produce"/>-->
         </div>
         <div class="body">
            <p>
   ...
   <xsl:template
      match="formal-name | description | remarks | constraint" />
